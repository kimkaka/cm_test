...
public class CommonFunctions {
	...
	public void initData() throws IOException {
		...
		FileInputStream Ist = new FileInputStream(System.getProperty("user.dir") + "/src/com/test/SYSPARAM.properties");
		SYSPARAM.load(Ist);
	...
	public void initBrowser() {
		/*
		 * // Check If browser Is already opened if (!browserAlreadyOpen) { if
		 * (SYSPARAM.getProperty("BrowserToTestIn").equals("MFF")) {
		 * System.setProperty("webdriver.gecko.driver",
		 * "D:\\Automation\\Setup\\geckodriver.exe"); driver = new FirefoxDriver(); }
		 * else if (SYSPARAM.getProperty("BrowserToTestIn").equals("CHRM")) { driver =
		 * new ChromeDriver(); } driver.manage().timeouts().implicitlyWait(15,
		 * TimeUnit.SECONDS); browserAlreadyOpen = true; }
		 */
